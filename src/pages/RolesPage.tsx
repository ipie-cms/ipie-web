 ...
 */
function groupByResource(permissions: PermissionResponse[]): [string, PermissionResponse[]][] {
    ...
    <div className="flex h-full flex-col gap-6 overflow-y-auto p-6">
      <Card className="flex flex-col">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button type="button" onClick={() => setCreatingPermission(true)}>
            New permission
          </Button>
        </CardHeader>
        <CardContent>
          {permissionsLoading && <p className="text-muted-foreground text-sm">Loading…</p>}
      ...
      {creatingPermission && (
        <PermissionCreateDialog onClose={() => setCreatingPermission(false)} />
      )}
    </div>
  )
}
